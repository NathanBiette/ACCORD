...
public class RepeatAction2{
	 ...
	 private ArrayList<Integer> ArrayList;//Vecteur mouvement, initialisée à 0 0
	...
	public RepeatAction2() throws IOException {
		...
		//Chargement de la library
	     ...
	     if (os.toUpperCase().contains("WINDOWS")) {
	      ...
	      t.schedule(new MonAction(), 0, 1*100);//Répétition de MonAction toutes les 2 secondes
		    }
	...
	class MonAction extends TimerTask {
	    ...
	    public void run() {
	      ...
	      if (repetition) {
	        try {
				ArrayList=Surf(ArrayList);//On actualise le vecteur mouvement, le cadre, l'image à analyser
			} catch (IOException e) {
	...
	public final ArrayList<Integer> Surf(ArrayList<Integer> vectIfNotWorking) throws IOException{ 
		...
		if (debug){
	      ...
	      //Tri et estimation du vecteur mouvement en retirant les matching aberrants
	       for (int i = 0; i < goodMatchesListBeforeTri.size(); i++) {
             ...
             Point ptarrivee=scnKeypointlist.get(goodMatchesListBeforeTri.get(i).trainIdx).pt;
           	...
           	int lx= (int)(ix+ptarrivee.x-(x+ptdepart.x));//Mediane plutôt que moyenne
         ...
         vect.add(new Integer((int) (vyfinal/comptVFiltre)));}
          //Si au moins un bon matching filtré est détecté alors on rentre les nouvelles coordonnées du mouvement repéré
     else {
   	  if (debug){
         ...
         Highgui.imwrite("C://Users//dimitri//Pictures//RectangleArrivee.jpg", retour);//Dessine l'image d'arrivée ainsi que les 2 rectangles
         Highgui.imwrite("C://Users//dimitri//Pictures//RectangleDepart.jpg", imageDepartNonCroppe);//Dessine l'image de départ ainsi que les 2 rectangles
